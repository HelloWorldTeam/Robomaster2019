...
typedef enum {
  RUNMODE_PLAY =
      0,  //正常预览，捕获到图像就显示。（如果相机处于触发模式，则会等待触发帧的到来）
...
typedef enum {
  ...
  FILE_BMP = 2,  // BMP
  FILE_RAW =
      4,  //相机输出的bayer格式文件,对于不支持bayer格式输出相机，无法保存为该格式
  FILE_PNG = 8  // PNG
} emSdkFileType;
...
typedef struct {
  ...
  char acFriendlyName
      [32];  // 产品昵称，用户可自定义改昵称，保存在相机内，用于区分多个相机同时使用,可以用CameraSetFriendlyName接口改变该昵称，设备重启后生效。
...
typedef struct {
  ...
  UINT uSkipModeMask;  // SKIP模式掩码，为0，表示不支持SKIP
                       // 。bit0为1,表示支持SKIP 2x2 ;bit1为1，表示支持SKIP
                       // 3x3....
  UINT uBinSumModeMask;  // BIN(求和)模式掩码，为0，表示不支持BIN
                         // 。bit0为1,表示支持BIN 2x2 ;bit1为1，表示支持BIN
                         // 3x3....
  UINT uBinAverageModeMask;  // BIN(求均值)模式掩码，为0，表示不支持BIN
                             // 。bit0为1,表示支持BIN 2x2 ;bit1为1，表示支持BIN
                             // 3x3....
...
//相机的分辨率描述
typedef struct {
  INT iIndex;  // 索引号，[0,N]表示预设的分辨率(N
               // 为预设分辨率的最大个数，一般不超过20),OXFF
               // 表示自定义分辨率(ROI)
  char acDescription
      [32];  // 该分辨率的描述信息。仅预设分辨率时该信息有效。自定义分辨率可忽略该信息
  UINT
      uBinSumMode;  // BIN(求和)的模式,范围不能超过tSdkResolutionRange中uBinSumModeMask
  UINT
      uBinAverageMode;  // BIN(求均值)的模式,范围不能超过tSdkResolutionRange中uBinAverageModeMask
  UINT
      uSkipMode;  // 是否SKIP的尺寸，为0表示禁止SKIP模式，范围不能超过tSdkResolutionRange中uSkipModeMask
  UINT uResampleMask;  // 硬件重采样的掩码
...
typedef struct {
  ...
  float fAnalogGainStep;  //模拟增益每增加1，对应的增加的放大倍数。例如，uiAnalogGainMin一般为16，fAnalogGainStep一般为0.125，那么最小放大倍数就是16*0.125
                          //= 2倍
  UINT
      uiExposeTimeMin;  //手动模式下，曝光时间的最小值，单位:行。根据CameraGetExposureLineTime可以获得一行对应的时间(微秒),从而得到整帧的曝光时间
...
typedef struct {
  ...
  char acDescription[32];  //描述信息
  UINT
      iMediaType;  //对应的图像格式编码，如CAMERA_MEDIA_TYPE_BAYGR8，在本文件中有定义。
} tSdkMediaType;
...
typedef struct {
  BOOL
      bMonoSensor;  //表示该型号相机是否为黑白相机,如果是黑白相机，则颜色相关的功能都无法调节
  ...
  BOOL
      bForceUseDeviceIsp;  // bDeviceIsp和bForceUseDeviceIsp同时为TRUE时，表示强制只用硬件ISP，不可取消。
...
typedef struct {
  ...
  tSdkFrameSpeed *pFrameSpeedDesc;  // 可调节帧速类型，对应界面上普通 高速
                                    // 和超级三种速度设置
  ...
  BOOL
      bParamInDevice;  // 指示该设备是否支持从设备中读写参数组。1为支持，0不支持。

  tSdkAeAlgorithm *pAeAlmSwDesc;  // 软件自动曝光算法描述
  int iAeAlmSwDesc;               // 软件自动曝光算法个数

  tSdkAeAlgorithm
      *pAeAlmHdDesc;  // 硬件自动曝光算法描述，为NULL表示不支持硬件自动曝光
  int iAeAlmHdDesc;  // 硬件自动曝光算法个数，为0表示不支持硬件自动曝光

  tSdkBayerDecodeAlgorithm
      *pBayerDecAlmSwDesc;  // 软件Bayer转换为RGB数据的算法描述
  int iBayerDecAlmSwDesc;   // 软件Bayer转换为RGB数据的算法个数

  tSdkBayerDecodeAlgorithm *
      pBayerDecAlmHdDesc;  // 硬件Bayer转换为RGB数据的算法描述，为NULL表示不支持
...
//图像捕获的回调函数定义
typedef void (*CAMERA_SNAP_PROC)(CameraHandle hCamera, BYTE *pFrameBuffer,
                                 tSdkFrameHead *pFrameHead, PVOID pContext);

// SDK生成的相机配置页面的消息回调函数定义
typedef void (*CAMERA_PAGE_MSG_PROC)(CameraHandle hCamera, UINT MSG,
                                     UINT uParam, PVOID pContext);
...
#define CAMERA_MEDIA_TYPE_MONO8 \
  (CAMERA_MEDIA_TYPE_MONO | CAMERA_MEDIA_TYPE_OCCUPY8BIT | 0x0001)
