...
CAN::CAN(const char *port_name) {
  int port_handle;
  struct sockaddr_can addr;
  struct ifreq ifr;
  /* create socket */
  if ((port_handle = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0) {
  ...
  /* assign can device */
  ioctl(port_handle, SIOCGIFINDEX, &ifr);
  addr.can_family = AF_CAN;
  addr.can_ifindex = ifr.ifr_ifindex;
  /* bind can device */
  if (bind(port_handle, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
  ...
  if (setsockopt(port_handle, SOL_CAN_RAW, CAN_RAW_FILTER, &rfilter,
                 sizeof(rfilter)) < 0) {
...
int CAN::send(double yaw, double pitch) {
  ...
  printf("Can send: %d ID=%#x data_length=%d data=", port_handle, frame.can_id,
         frame.can_dlc);
...
int CAN::send(double X, double Y, double Z) {
  /* configure can_id and can data length */
  static struct can_frame frame;
  frame.can_id = 0x2FF;
  frame.can_dlc = 8;
...
int CAN::send(double X, double Y, double Z, int type) {
  /* configure can_id and can data length */
  static struct can_frame frame;
  frame.can_id = 0x2FF;
  frame.can_dlc = 8;
...
int CAN::receive(char *buffer) {
  ...
  if (frame.can_id == 0x3FF) {  // 手动过滤一下
    printf("Can receive: %d ID=%#x data_length=%d data=", port_handle,
